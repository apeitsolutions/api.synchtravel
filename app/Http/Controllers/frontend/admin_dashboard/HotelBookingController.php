...
class HotelBookingController extends Controller
{
    ...
    public function hotels_search(Request $request)
    {
       ...
       $rooms=$request->rooms;
       $rooms_search=$request->rooms;
       $adults=$request->adults;
       $children=$request->children;
       $destination=$request->destination;
       $nationality=$request->nationality;
       $multipal_rooms=$request->multipal_rooms;
       $multipal_rooms_dec=json_decode($multipal_rooms);
      ...
      //print_r($destination);die();
       ...
       ->where('availible_from','<=',$checkin)
       ->Where('availible_to','>=',$checkout)
      // whereJsonLength('more_room_type_details->more_room_av_from', '<=', $checkin)->whereJsonLength('more_room_type_details->more_room_av_to', '>=', $checkout)
       
    //   ->where('availible_from','<=',$checkin)
    //   ->Where('availible_from','<',$checkout)
    //   >where('availible_to','>=',$checkout)
    //   ->Where('availible_to','<',$checkin)
    ...
    //   ->whereJsonContains('more_room_type_details',['more_room_av_from'=> '<=',$checkin,'more_room_av_to'=>'>=',$checkout])
    //   ->whereJsonContains('more_room_type_details',['more_quantity'=> '>=',$rooms])
    //   ->whereJsonContains('more_room_type_details',['more_max_adults'=> '>=',$adults])
    //   ->whereJsonContains('more_room_type_details',['more_max_childrens'=> '>=',$children])
    //   ->orwhere('max_child','<=',$children)
       ...
       {
           $rooms_data=array();
           foreach($multipal_rooms_dec as $multipal_room)
           {
         $rooms_data[] = DB::table('rooms')
                    ->where('availible_from','<=',$checkin)
                   ->Where('availible_to','>=',$checkout)
                    ->where('quantity','>=',$multipal_room->Room)
                    ->where('max_adults','>=',$multipal_room->Adults)
                    ->where('max_child','=',$multipal_room->Children)
                    ->where('owner_id',$userData->id)
                    ->get()->toArray();
           }
       }
         ...
         {
         $getdata=array();
         foreach($rooms_data as $rooms_datas)
         {
            foreach($rooms_datas as $rooms_all_data)
            {
                $getdata[]=$rooms_all_data;
            }
         }
         }
...
// die();
      ...
      $hotels=array();
    $room_images_gallary=array();
       foreach($rooms as $res)
       {
       
          $hotels[] = DB::table('hotels')->where('id',$res->hotel_id)->where('property_city',$destination)->where('owner_id',$userData->id)->first();   
       
        
         $room_images_gallary[] = DB::table('room_galleries')->where('room_id',$res->id)->first();
            
       }
       
    //print_r($hotels);die(); 
        $result=array_unique($hotels, SORT_REGULAR);
        ...
        return response()->json(
            [
                ...
                'hotel'=>$result,
                'rooms'=>$hotel_rooms,
                'room_images_gallary'=>$room_images_gallary
            ]
            );
        }else{
             return response()->json(
    ...
    public function add_lead_passengar(Request $request)
    {
        ...
        $add_lead_passengar = DB::table('hotel_booking')->where('search_id',$search_id)->update(
        [
        'lead_passenger_details'=>$lead_passenger_details,
  ...
  public function hotel_cart(Request $request)
   ...
   public function tbo_get_hotel_code_list(Request $request)
    {
        $CityName=$request->CityName;
        $tbo_hotel_codes=\DB::table('synchron_mata.tbo_hotel_details')->where('city_name',$CityName)->get();
