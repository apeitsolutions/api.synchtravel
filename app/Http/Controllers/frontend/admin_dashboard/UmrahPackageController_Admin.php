...
class UmrahPackageController_Admin extends Controller
{
  ...
  // Tour Bookings
     // Tour Bookings
        public function view_ternary_bookings_Admin(Request $req){
            $all_Users  = DB::table('customer_subcriptions')->get();
            $data1      = DB::table('tours_bookings')->join('customer_subcriptions','tours_bookings.customer_id','customer_subcriptions.id')
                            ->join('cart_details','tours_bookings.id','cart_details.booking_id')->where('pakage_type','tour')->orderBy('tours_bookings.id', 'DESC')->get();
            $data       = $data1;
            // dd($data);
            return view('template/frontend/userdashboard/pages/tour/view_ternary_bookings',compact('data','all_Users'));
        }
...
$id=$request->id;
  ...
  public function create_excursion(Request $request)
  {
    ...
    public function create_excursion2(Request $request){
        ...
        return response()->json(['message'=>'success','visa_supplier'=>$visa_supplier,'visa_types'=>$visa_types,'flight_suppliers'=>$flight_suppliers,'all_flight_routes'=>$all_flight_routes,'tranfer_supplier'=>$tranfer_supplier,'destination_details'=>$destination_details,'supplier_detail'=>$supplier_detail,'mange_currencies'=>$mange_currencies,'transfer_Vehicle'=>$transfer_Vehicle,'currency_Symbol'=>$currency_Symbol,'customer'=>$customer,'attributes'=>$attributes,'others_providers_list'=>$others_providers_list,'categories'=>$categories,'all_countries'=>$all_countries,'all_countries_currency'=>$all_countries_currency,'payment_gateways'=>$payment_gateways,'payment_modes'=>$payment_modes]);
    }
    
    public function view_tour_Admin(Request $request){
        $all_Users      = DB::table('customer_subcriptions')->get();
        $pakage_type    = 'tour';
        $tours          = DB::table('tours')->join('tours_2','tours.id','tours_2.tour_id')->get();
        $final_data     = DB::table('cart_details')->where('pakage_type',$pakage_type)->get();;
        $booking_Id     = DB::table('tours')->join('tours_2','tours.id','tours_2.tour_id')->join('cart_details','tours.id','cart_details.tour_id')->select('cart_details.tour_id','cart_details.booking_id')->get();
        $data           = [];
        foreach($tours as $tours_res){
            $tour_book = false;
            foreach($final_data as $book_res){
                if($tours_res->tour_id == $book_res->tour_id){
                    // dd($tours_res);
                    $tour_book = true;
                }
            }
            $single_tour = [
                'id'                => $tours_res->tour_id,
                'generate_id'       => $tours_res->generate_id,
                ...
                'flight_id'         => $tours_res->flight_id ?? '',
                'customer_id'       => $tours_res->customer_id,
            ];
            array_push($data,$single_tour);
        }
        
        return view('template/frontend/userdashboard/pages/tour/view_tour',compact(['tours','data','final_data','all_Users']));
    ...
    public function submit_tour(Request $request){
        ...
        try {
            ...
            $request->destination_details_more = json_encode($accomodation_more_data);
            ...
            $tour->flight_Included = $request->flight_Included;
    ...
    public function edit_tour(Request $request){
        ...
        return response()->json(['message'=>'success','visa_supplier'=>$visa_supplier,'visa_types'=>$visa_types,'flight_suppliers'=>$flight_suppliers,'all_flight_routes'=>$all_flight_routes,'tranfer_supplier'=>$tranfer_supplier,'destination_details'=>$destination_details,'supplier_detail'=>$supplier_detail,'visa_type_get'=>$visa_type_get,'mange_currencies'=>$mange_currencies,'airline_Name'=>$airline_Name,'other_Hotel_Name' => $other_Hotel_Name,'tours'=>$tours,'currency_Symbol'=>$currency_Symbol,'payment_modes'=>$payment_modes,'payment_gateways'=>$payment_gateways,'customer'=>$customer,'attributes'=>$attributes,'categories'=>$categories,'all_countries'=>$all_countries,'all_countries_currency'=>$all_countries_currency,'bir_airports'=>$bir_airports]);
    }
    ...
    public function submit_edit_tour(Request $request){
        ...
        try {
            ...
            { 
                ...
                $activities->flight_Included        = $request->flight_Included;
            ...
            $new_transfer_det = json_decode($request->transportation_details);
                ...
                $tour_location = json_encode($request->tour_location);
                ...
                if($request->package_update_type == 'old'){
                    ...
                    $tour_bactches=DB::table('tour_batchs')->where('generate_id',$request->prev_generate_code)->update([
                        ...
                        'whats_excluded'=> $request->whats_excluded,
                        'payment_gateways'=> $request->payment_gateways,
                        'payment_modes'=> json_encode($request->payment_modes),
                ...
                if($previous_flight_route != $request->flight_route_id_occupied){
                    ...
                    $route_obj = DB::table('flight_rute')->where('id',$previous_flight_route)->select('id','dep_supplier','flights_per_person_price','flights_per_child_price','flights_per_infant_price')->first();
                    $route_obj_new = DB::table('flight_rute')->where('id',$request->flight_route_id_occupied)->select('id','dep_supplier','flights_per_person_price','flights_per_child_price','flights_per_infant_price')->first();
                    ...
                    foreach($Cart_details as $cart_res){
                        ...
                        $supplier_data = DB::table('supplier')->where('id',$previous_flight_sup)->first();
                        ...
                        $supplier_data = DB::table('supplier')->where('id',$request->flight_supplier)->first();
    ...
    public function super_admin_index(Request $request){
        ...
        if(isset($flight_supplier_detail) && $flight_supplier_detail != null && $flight_supplier_detail != ''){
            foreach($flight_supplier_detail as $flight_supplier_detailS){
                ...
                // Invoice
                $all_invoices           = DB::table('add_manage_invoices')->where('customer_id',$request->customer_id)->whereJsonContains('services',['flights_tab'])->select('flight_supplier')->get();
                ...
                // Package
                $all_tour_bookings      = DB::table('cart_details')->where('client_id',$request->customer_id)->select('cart_total_data')->get();
                ...
                $supplier_TBCA = [
                    ...
                    'supplier_balance'          => $flight_supplier_detailS->balance,
        ...
        $hotel_supplier_detail          = DB::table('rooms_Invoice_Supplier')->where('customer_id',$request->customer_id)->orderBy('id', 'DESC')->take(5)->get();
            ...
            foreach($hotel_supplier_detail as $hotel_supplier_detailS){
                ...
                if(isset($all_rooms) && $all_rooms != null && $all_rooms != ''){
                    foreach($all_rooms as $all_roomsS){
                        // Invoice
                        $rooms_bookings_details_I = DB::table('rooms_bookings_details')->where('room_id',$all_roomsS->id)->where('booking_from',$invoice_N)->select('quantity')->get();
                        ...
                        // Package
                        $rooms_bookings_details_P = DB::table('rooms_bookings_details')->where('room_id',$all_roomsS->id)->where('booking_from',$package_N)->select('quantity')->get();
                ...
                $supplier_TBCA = [
                    ...
                    'supplier_balance'          => $hotel_supplier_detailS->balance,
        ...
        if(isset($transfer_supplier_detail) && $transfer_supplier_detail != null && $transfer_supplier_detail != ''){
            foreach($transfer_supplier_detail as $transfer_supplier_detailS){
                ...
                $transfer_supplier_ledger = DB::table('transfer_supplier_ledger')->where('customer_id',$request->customer_id)->where('supplier_id',$transfer_supplier_detailS->id)->count();
                if(isset($transfer_supplier_ledger) && $transfer_supplier_ledger != null && $transfer_supplier_ledger != ''){
                    $transfer_supplier_bookings_count   = $transfer_supplier_bookings_count + $transfer_supplier_ledger;
                    $all_transfer_supplier_count        = $all_transfer_supplier_count + $transfer_supplier_ledger;
                }
                
                $supplier_TBCA = [
                    'supplier_id'            => $transfer_supplier_detailS->id,
                    'supplier_name'          => $transfer_supplier_detailS->room_supplier_name,
                    'supplier_balance'       => $transfer_supplier_detailS->balance,
                    'supplier_bookings_count'=> $transfer_supplier_bookings_count,
                ];
                array_push($transfer_supplier_detail_array,$supplier_TBCA);
            }
            // dd($transfer_supplier_detail_array);
        }
        
        if($all_flight_supplier_count > $all_hotel_supplier_count){
            $supplier_detail = $flight_supplier_detail_array;
        }elseif($all_hotel_supplier_count > $all_transfer_supplier_count){
            $supplier_detail = $hotel_supplier_detail_array;
        }else{
            $supplier_detail = $transfer_supplier_detail_array;
        }
        
        $agent_detail               = DB::table('Agents_detail')->where('customer_id',$request->customer_id)->orderBy('id', 'DESC')->take(5)->get();
        $agent_detail_array         = [];
        if(isset($agent_detail) && $agent_detail != null && $agent_detail != ''){
            foreach($agent_detail as $agent_detailS){
                // Invoice
                $agent_total_bookings_count     = 0;
                $all_invoices                   = DB::table('add_manage_invoices')->where('customer_id',$request->customer_id)->select('agent_id')->get();
                foreach($all_invoices as $all_invoicesS){
                    $agent_id = $all_invoicesS->agent_id;
                    if(isset($agent_id) && $agent_id != null && $agent_id != ''){
                        if($agent_id == $agent_detailS->id){
                ...
                $agent_TBCA = [
                    ...
                    'balance'                       => $agent_detailS->balance,
                    'agent_total_bookings_count'    => $agent_total_bookings_count,
                ];
                array_push($agent_detail_array,$agent_TBCA);
                
            }
            // dd($agent_detail_array);
        }
        
        $customer_detail            = DB::table('booking_customers')->where('customer_id',$request->customer_id)->orderBy('id', 'DESC')->take(5)->get();
        ...
        if(isset($customer_detail) && $customer_detail != null && $customer_detail != ''){
            foreach($customer_detail as $customer_detailS){
                ...
                $all_invoices                       = DB::table('add_manage_invoices')->where('customer_id',$request->customer_id)->select('booking_customer_id')->get();
                foreach($all_invoices as $all_invoicesS){
                    $booking_customer_id = $all_invoicesS->booking_customer_id;
                    if(isset($booking_customer_id) && $booking_customer_id != null && $booking_customer_id != ''){
                        if($booking_customer_id == $customer_detailS->id){
                ...
                foreach($all_tour_bookings as $all_tour_bookingsS){
                    ...
                    if(isset($cart_total_data_E) && $cart_total_data_E != null && $cart_total_data_E != ''){
                        ...
                        if(isset($cart_total_data->customer_id) && $cart_total_data->customer_id != null && $cart_total_data->customer_id != '' && $cart_total_data->customer_id > 0){
                            if($cart_total_data->customer_id == $agent_detailS->id){
                                $customer_total_bookings_count = $customer_total_bookings_count + 1;
                            }
                        }
                    }
                ...
                $customer_TBCA = [
                    ...
                    'balance'                       => $customer_detailS->balance,
