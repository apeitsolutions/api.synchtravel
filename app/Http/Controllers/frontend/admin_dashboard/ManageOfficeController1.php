...
class ManageOfficeController extends Controller
{
        ...
        try {
                 ...
                 if(isset($new_acc_res->more_hotelRoom_type_id) AND !empty($new_acc_res->more_hotelRoom_type_id)){
                     if(!$ele_found){
                         $room_data = DB::table('rooms')->where('id',$new_acc_res->more_hotelRoom_type_id)->first();
                            ...
                            $room_data_update = DB::table('rooms')->where('id',$new_acc_res->more_hotelRoom_type_id)->update([
                            'booked'=>$update_booked
                            ]);
            ...
            if(isset($flights_Pax_details) && $flights_Pax_details != null && $flights_Pax_details != ''){
                foreach($flights_Pax_details as $value){
                    ...
                    DB::table('flight_seats_occupied')
                        ->where('booking_id',$invoice_data->id)
                        ->where('type','Invoice')
                        ->delete();
    ...
    public function add_Agents(Request $request){
        ...
        return response()->json(['status'=>'success','message1'=>'success','message'=>'Agent Added Successful!']);
    }
    ...
    public function agents_new_slot(Request $req){
        ...
        return response()->json(['message'=>'success','all_currency'=>$all_currency,'agent_Details'=>$agent_Details,'mange_currencies'=>$mange_currencies,'agent_Slots'=>$agent_Slots,'visa_suppliers'=>$visa_suppliers]);
    }
    ...
    public function create_Invoices(Request $req){
        ...
        return response()->json(['message'=>'success','agent_Slots'=>$agent_Slots,'groups_Detail'=>$groups_Detail,'vehicle_category'=>$vehicle_category,'tranfer_vehicle'=>$tranfer_vehicle,'tranfer_destination'=> $tranfer_destination,'tranfer_supplier'=> $tranfer_supplier,'tranfer_company'=>$tranfer_company,'mange_currencies'=>$mange_currencies,'visa_supplier'=>$visa_supplier,'visa_types'=>$visa_types,'flight_suppliers'=>$flight_suppliers,'all_flight_routes'=>$all_flight_routes,'all_curr_symboles'=>$all_curr_symboles,'tranfer_supplier'=>$tranfer_supplier,'supplier_detail'=>$supplier_detail,'mange_currencies'=>$mange_currencies,'user_hotels'=>$user_hotels,'Agents_detail'=>$Agents_detail,'currency_Symbol'=>$currency_Symbol,'customer'=>$customer,'attributes'=>$attributes,'categories'=>$categories,'all_countries'=>$all_countries,'all_countries_currency'=>$all_countries_currency,'bir_airports'=>$bir_airports,'payment_gateways'=>$payment_gateways,'payment_modes'=>$payment_modes,'customers_data'=>$customers_data]);
    }
    ...
    public function add_Invoices_CP(Request $req){
        ...
        try {
            ...
            if(isset($agent_data)){
                // echo "Enter hre ";
                $agent_balance = $agent_data->balance + $insert->total_sale_price_AC;
