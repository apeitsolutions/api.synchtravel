...
class TestHotelBookingControllerApi extends Controller
{
    //
    ...
    function search_hotels(Request $request){
            ...
            foreach($all_hotels as $hotel_res){
                ...
                foreach($rooms_adults as $index => $adult_res){
                    $rooms = DB::table('rooms')
                                ...
                                ->where('max_child',$rooms_childs[$index])
                    ...
                    if(count($rooms)){
                        foreach($rooms as $room_res){
                            if (in_array($room_res->id, $rooms_ids)) {
                                $room_index = array_search($room_res->id, $rooms_ids);
                                // print_r($rooms_qty);
                               
                                $room_qty = $rooms_qty[$room_index];
                                $room_qty--;
                                if($room_qty > 0){
                                    $rooms_qty[$room_index] = $room_qty;
                                ...
                                // print_r($rooms_qty);
                                // echo in_array($room_res->id, $rooms_ids);
                            } else {
                                
                                $rooms_ids[] = $room_res->id;
                                $rooms_qty[] = $room_res->quantity - $room_res->booked;
                                $rooms_found[] = $room_res;
                                $counter++;
                            }
                        }
        ...
        {
            $url = "https://admin.synchronousdigital.com/synchtravelhotelapi/travellandaapi.php";
        ...
        function hotelbedssearch($newstart, $newend, $res_hotel_beds, $lat, $long){
            $url = "https://admin.synchronousdigital.com/synchtravelhotelapi/hotelbedsapi.php";
        ...
        if(isset($selected_hotel->hotel_provider)){
            ...
            if($selected_hotel->hotel_provider == 'hotel_beds'){
                  ...
                  function hotelbedsgetBooking($roomRate){
                      $url = "https://admin.synchronousdigital.com/synchtravelhotelapi/hotelbedsapi.php";
                      $data = array('case' => 'getBooking', 'roomRate' => $roomRate);
                      //print_r($data);die();
                      Session::put('hotelbeds_search_request',json_encode($data));
                      $ch = curl_init();
                      curl_setopt($ch, CURLOPT_URL, $url);
                            ...
                            $result = DB::table('hotels_bookings')->insert([
                                 ...
                                 //added code by jamshaid cheena 08-06-2023
                                    ...
                                    $manage_customer_markups = DB::table('manage_customer_markups')->insert([
                                        ...
                                        $credit_data = DB::table('credit_limit_transections')->where('customer_id',$request->customer_id)->where('status',1)->first();
                                         $ramainAmount=$credit_data->remaining_amount - $request->creditAmount;
                                        $credit_limits = DB::table('credit_limit_transections')->where('customer_id',$request->customer_id)->where('status',1)->insert([
