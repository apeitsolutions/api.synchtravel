...
class HotelBookingControllerApi extends Controller
{
    //
    function pr($x){
    ...
    function search_hotels(Request $request){
        ...
        // ************************************************************ //
            ...
            $all_hotels = DB::table('hotels')->where('property_city',$hotel_city_changed)
                                             ->orWhere('property_city',$request_data->destination)
    ...
    function hotels_checkout_submit(Request $request){
        $hotel_request_data = json_decode($request->request_data);
        ...
        if($hotel_checkout_select->hotel_provider  == 'Custome_hotel'){
            ...
            try {   
                ...
                if(isset($hotel_checkout_select->rooms_list[0])){
                    if($hotel_checkout_select->rooms_list[0]->request_type == '0'){
    ...
    function proccess_non_refundable_booking(Request $request){
        $userData   = CustomerSubcription::where('Auth_key',$request->token)->select('id','status')->first();
    ...
    function on_request_checkout_submit(Request $request){
    ...
    // Confirm Processing
    function makePayment_on_request(Request $request){
        $result             = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->update([
            'status_MP' => 1,
            'status'    => 'Room On Request',
        ]);
        $booking_data       = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->first();
        return response()->json([
            'status'        => 'success',
    ...
    function confrim_on_request(Request $request){
        $booking_data               = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->first();
        $reservation_data           = json_decode($booking_data->reservation_response);
        $reservation_data->status   = 'Confirmed';
        if(isset($reservation_data->hotel_details->rooms)){
            foreach($reservation_data->hotel_details->rooms as $index => $room_res){
                $reservation_data->hotel_details->rooms[$index]->room_stutus = 'Confirmed';
            }
        }
    ...
    function reject_on_request(Request $request){
        $booking_data               = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->first();
        $reservation_data           = json_decode($booking_data->reservation_response);
    ...
    function confrim_on_request_AR(Request $request){
        $currentDateTime    = Carbon::now();
        $updatedDateTime    = $currentDateTime->addHours($request->hours_AR);
        $formattedDateTime  = $updatedDateTime->format('d-m-Y H:i:s');
        $result             = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->update([
            'expire_at'     => $formattedDateTime,              
            'status_AR'     => 1,
        ]);
    ...
    function reject_on_request_AR(Request $request){
        $currentDateTime    = Carbon::now();
        $updatedDateTime    = $currentDateTime->addHours($request->hours_AR);
        $formattedDateTime  = $updatedDateTime->format('d-m-Y H:i:s');
        $result             = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->update([
    ...
    function in_process_on_request(Request $request){
        // DB::beginTransaction();
        try {
            ...
            // dd($reservation_request,$hotel_checkout_select,$rooms_list);
            
            // Supplier and Room Data
            foreach($rooms_list as $room_res){
                $room_data                      = DB::table('rooms')->where('id',$room_res->booking_req_id)->first();
                $rooms_qty                      = $room_res->selected_qty;
                $total_booked                   = $room_data->booked + $rooms_qty;
                $quantity                       = $room_data->quantity - $total_booked;
                DB::table('rooms_bookings_details')->insert([
                    ...
                    'booking_from'              => 'website',
                    'quantity'                  => $rooms_qty,
                    'booking_id'                => $request->invoice_no,
                    'date'                      => date('d-m-Y'),
                    'check_in'                  => $hotel_checkout_select->checkIn,
                    'check_out'                 => $hotel_checkout_select->checkOut,
                ]);
                
                DB::table('rooms')->where('id',$room_res->booking_req_id)->update(['booked'=>$total_booked,'quantity'=>$quantity]);
                
                // dd($room_data->room_supplier_name_AR);
                
                if($room_data->room_supplier_name_AR == 135){
                    // dd('IF');
                    
                    $supplier_data = DB::table('rooms_Invoice_Supplier')->where('id',135)->select('id','balance','payable')->first();
                    if(isset($supplier_data)){
                        $total_price                = $room_res->rooms_total_price;
                        $supplier_balance           = $supplier_data->balance;
                        $supplier_payable_balance   = $supplier_data->payable + $total_price;
                        ...
                        DB::table('hotel_supplier_ledger')->insert([
                            ...
                            'customer_id'           => $userData->id,
                            'date'                  => date('Y-m-d'),
                            'website_booking_id'    => $request->invoice_no,
                            'available_from'        => $hotel_checkout_select->checkIn,
                            'available_to'          => $hotel_checkout_select->checkOut,
                            'room_quantity'         => $room_res->selected_qty,
                        ]);
                        
                        DB::table('rooms_Invoice_Supplier')->where('id',135)->update([
                            'balance'               => $supplier_balance,
                            'payable'               => $supplier_payable_balance
                        ]);
                    }
    ...
    function hotel_invoice(Request $request){
        $booking_data = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->first();
    ...
    function payment_cp_update(Request $request){
        $booking_data = DB::table('hotels_bookings')->where('invoice_no',$request->invoice_no)->update([
            'payment_cp'=>1
            ]);
        return response()->json([
                'status' => 'success',
