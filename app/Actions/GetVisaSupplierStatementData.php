...
class GetVisaSupplierStatementData
{
       ...
       if(!empty($startDate)){
                        ...
                        $supplier_invoices = DB::table('add_manage_invoices')
                                                        ->where(function($query) use($supplierId){
                                                            $query->whereJsonContains('all_visa_price_details',['visa_supplier_id'=>"".$supplierId.""])
                                                            ->OrWhereJsonContains('all_visa_price_details',['visa_supplier_id'=>$supplierId]);
                                                        })
                            ...
                            foreach($supplier_invoices as $supplier_res){
                                ...
                                if(isset($all_visa_price_details)){
                                    foreach($all_visa_price_details as $visa_detial_res){
                                        if($visa_detial_res->visa_supplier_id == $supplierId){
                        ...
                        $make_payments_data = DB::table('make_payments_details')
                         ...
                         $supplier_invoices = DB::table('add_manage_invoices')
                                                         ->whereJsonContains('all_visa_price_details',['visa_supplier_id'=>"".$supplierId.""])
                                                         ->OrWhereJsonContains('all_visa_price_details',['visa_supplier_id'=>$supplierId])
                                                         ->select('id as invoice_no','all_visa_price_details','created_at')
                                                         ->get();
                ...
                $suppliers_data     = DB::table('visa_Sup')->where('id',$supplierId)->first();
